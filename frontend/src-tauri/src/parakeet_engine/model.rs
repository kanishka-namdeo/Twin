...
impl ParakeetModel {
    ...
    fn init_session<P: AsRef<Path>>(
        ...
        #[cfg(feature = "cuda")]
        {
            ...
            log::info!("Parakeet: CUDA execution provider enabled, using GPU acceleration");
        ...
        log::info!("Creating ONNX session for Parakeet model '{}'...", model_filename);
        let session = builder.commit_from_file(model_dir.as_ref().join(&model_filename))?;
        log::info!("ONNX session created successfully for '{}'", model_filename);
