...
#[tauri::command]
pub async fn skip_onboarding_cmd<R: Runtime>(
    app: AppHandle<R>,
) -> Result<(), String> {
    info!("Skipping onboarding - marking as completed without model downloads");
    ...
    status.completed = true;
    status.current_step = 0; // Indicates skipped
    status.skipped = true;
    // Do NOT set model_status to downloaded - models are not installed
    // Do NOT save any model configs

    save_onboarding_status(&app, &status)
        .await
        .map_err(|e| format!("Failed to save skipped onboarding status: {}", e))?;

    info!("Onboarding skipped successfully");
    Ok(())
}
