...
pub struct ParallelProcessor {
    ...
    lifecycle_manager: Option<Arc<AppLifecycleManager>>,
...
impl ParallelProcessor {
    ...
    pub async fn start_processing(
        ...
        *self.is_paused.write().await = false;
        // Create a fresh cancellation token for this processing session
        ...
        if let Some(ref lifecycle_manager) = self.lifecycle_manager {
            ...
            lifecycle_manager
                .register(
                    "parallel_processor_workers",
                    crate::lifecycle::resource::CleanupPriority::Secondary,
                    Box::new(move || {
                        Box::pin(async move {
                            log::info!("Lifecycle: cancelling parallel processor workers...");
                            cancellation_token.cancel();

                            // Wait for worker tasks to complete (with timeout)
                            // This ensures GPU contexts are properly dropped
                            ...
                            for (i, handle) in handles.drain(..).enumerate() {
                                match tokio::time::timeout(
                                    ...
                                    Ok(Ok(())) => log::info!(
                                        "Lifecycle: worker {}/{} completed", i + 1, total
                                    ),
                                    Ok(Err(e)) => log::warn!(
                                        "Lifecycle: worker {}/{} join error: {}", i + 1, total, e
                                    ),
                                    Err(_) => log::warn!(
                                        "Lifecycle: worker {}/{} timed out during shutdown", i + 1, total
                                    ),
                                }
                            }
                            log::info!("Lifecycle: parallel processor workers cleaned up");
                            Ok(())
                        })
                    }),
                )
    ...
    async fn create_worker(&self, worker_id: u32, model_name: String) -> Result<Worker> {
        ...
        let handle = tokio::spawn(async move {
            // Wrap the worker logic in an inner async fn-style block
            let result: Result<()> = async move {
                ...
                loop {
                    ...
                    // Wait if paused
                    while *is_paused.read().await && !cancellation_token.is_cancelled() {
        ...
        // Store the JoinHandle in worker_handles for lifecycle cleanup
        {
            let mut handles = worker_handles.lock().await;
            handles.push(handle);
        }
    ...
    pub async fn stop_processing(&mut self) {
        ...
        for (i, handle) in handles.drain(..).enumerate() {
            match tokio::time::timeout(
                ...
                Ok(Ok(())) => info!("Worker {}/{} stopped cleanly", i + 1, total),
                Ok(Err(e)) => warn!("Worker {}/{} join error: {}", i + 1, total, e),
                Err(_) => warn!("Worker {}/{} timed out during stop", i + 1, total),
            }
        }
        drop(handles);
        ...
        if let Some(ref lifecycle_manager) = self.lifecycle_manager {
            lifecycle_manager.unregister("parallel_processor_workers").await;
        }
    ...
    pub async fn get_processing_status(&self) -> ProcessingStatus {
        ...
        ProcessingStatus {
            ...
            is_stopped: self.cancellation_token.is_cancelled(),
        }
    }

    /// Set the lifecycle manager for this processor
    pub fn set_lifecycle_manager(&mut self, manager: Arc<AppLifecycleManager>) {
...
impl Drop for ParallelProcessor {
    fn drop(&mut self) {
        // Cancel workers synchronously to prevent them from running after drop
        self.cancellation_token.cancel();

        // Note: We can't await worker handles in Drop, but the cancellation token
        // will cause workers to exit their loops on their next iteration.
        // The JoinHandles will be dropped, detaching the tasks.
        log::info!("ParallelProcessor dropped, workers cancelled via cancellation token");
    }
}
