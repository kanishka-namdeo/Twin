...
impl WhisperEngine {
    ...
    pub fn new_with_models_dir(models_dir: Option<PathBuf>) -> Result<Self> {
        ...
        // Guarded with Once to ensure thread-safe initialization
        ...
        match model_info.status {
            ModelStatus::Available => {
                ...
                let ctx = {
                    ...
                    log::info!("Creating WhisperContext with GPU acceleration: use_gpu={}, flash_attn={}", context_param.use_gpu, context_param.flash_attn);
                    WhisperContext::new_with_params(model_info.path.to_string_lossy().to_string(), context_param)
    ...
    pub async fn unload_model(&self) -> bool  {
        ...
        if unloaded {
            log::info!("Dropping WhisperContext, releasing GPU memory");
    ...
    pub async fn transcribe_audio_with_confidence(&self, audio_data: Vec<f32>, language: Option<String>) -> Result<(String, f32, bool)> {
        ...
        let (num_segments, state) = {
            ...
            let num_segments = state.full_n_segments() as i32;
        ...
        for i in 0..num_segments {
            ...
            let segment_text: String = match segment.to_str_lossy() {
    ...
    pub async fn download_model(&self, model_name: &str, progress_callback: Option<Box<dyn Fn(u8) + Send>>) -> Result<()> {
        ...
        // Use a helper so all error paths clean up active_downloads via this wrapper
        ...
        // On failure, reset model status to Missing so it can be retried
        if result.is_err() {
            let mut models = self.available_models.write().await;
            if let Some(model_info) = models.get_mut(model_name) {
                if matches!(model_info.status, ModelStatus::Downloading { .. }) {
