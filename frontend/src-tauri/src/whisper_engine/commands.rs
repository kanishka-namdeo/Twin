...
pub fn set_models_directory<R: Runtime>(app: &AppHandle<R>) {
    ...
    let mut guard = MODELS_DIR.lock().expect("models_dir poisoned");
    *guard = Some(models_dir);
}

/// Get the configured models directory
fn get_models_directory() -> Option<PathBuf> {
    MODELS_DIR.lock().expect("models_dir poisoned").clone()
}

#[command]
pub async fn whisper_init() -> Result<(), String> {
    let mut guard = WHISPER_ENGINE.lock().expect("whisper_engine poisoned");
    if guard.is_some() {
        return Ok(());
    }
...
pub async fn whisper_get_available_models() -> Result<Vec<ModelInfo>, String> {
    let engine = {
        let guard = WHISPER_ENGINE.lock().expect("whisper_engine poisoned");
        guard.as_ref().cloned()
    };
