...
#[tauri::command]
pub async fn initialize_parallel_processor<R: Runtime>(
    app: AppHandle<R>,
    ...
    let (mut processor, _event_receiver) = ParallelProcessor::new(
        config.clone(),
    ...
    if let Some(lifecycle_manager) = app.try_state::<std::sync::Arc<crate::lifecycle::AppLifecycleManager>>() {
        processor.set_lifecycle_manager(lifecycle_manager.inner().clone());
    }
