...
impl AudioPipeline {
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        // Get cancellation token from recording state for lifecycle coordination
        let cancellation_token = self.state.get_cancellation_token();
        ...
        loop {
            // Check if cancelled via token
            if cancellation_token.is_cancelled() {
                info!("Audio pipeline cancelled via cancellation token");
                break;
            }
