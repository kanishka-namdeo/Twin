...
impl AudioDeviceMonitor {
    ...
    async fn monitor_loop(
        ...
        let base_check_interval = Duration::from_secs(2); // Poll every 2 seconds

        loop {
            // Determine check interval based on device states
            // If any device is missing, check more frequently
            let has_missing = monitored_devices.iter().any(|d| d.consecutive_missing > 0);
            let check_interval = if has_missing {
            ...
            let _ = base_check_interval; // suppress unused warning
        }
    }
}
