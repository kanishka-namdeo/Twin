...
impl RecordingState {
    ...
    pub fn start_recording(&self) -> Result<()> {
        ...
        *self.recording_start.lock().expect("recording_start mutex poisoned") = Some(Instant::now());
        self.error_count.store(0, Ordering::SeqCst);
        self.recoverable_error_count.store(0, Ordering::SeqCst);
        *self.last_error.lock().expect("last_error mutex poisoned") = None;

        // Create new cancellation token for this recording session
        *self.cancellation_token.lock().expect("cancellation_token mutex poisoned") = CancellationToken::new();
        log::info!("Recording started with new cancellation token");
    ...
    pub fn stop_recording(&self) {
        ...
        // Cancel the token to signal all components to stop
        self.cancellation_token.lock().expect("cancellation_token mutex poisoned").cancel();
        log::info!("Recording stopped, cancellation token triggered");

        // Clear pause tracking when stopping
        *self.pause_start.lock().expect("pause_start mutex poisoned") = None;
        ...
        *self.audio_sender.lock().expect("audio_sender mutex poisoned") = None;
        ...
        *self.microphone_device.lock().expect("microphone_device mutex poisoned") = None;
        *self.system_device.lock().expect("system_device mutex poisoned") = None;
        *self.disconnected_device.lock().expect("disconnected_device mutex poisoned") = None;
    ...
    pub fn pause_recording(&self) -> Result<()> {
        ...
        *self.pause_start.lock().expect("pause_start mutex poisoned") = Some(Instant::now());
    ...
    pub fn resume_recording(&self) -> Result<()> {
        ...
        if let Some(pause_start) = self.pause_start.lock().expect("pause_start mutex poisoned").take() {
            let pause_duration = pause_start.elapsed();
            *self.total_pause_duration.lock().expect("total_pause_duration mutex poisoned") += pause_duration;
    ...
    /// Get a clone of the current cancellation token for use in other components
    pub fn get_cancellation_token(&self) -> CancellationToken {
        self.cancellation_token.lock().expect("cancellation_token mutex poisoned").clone()
    }

    /// Check if recording has been cancelled via token
    pub fn is_cancelled(&self) -> bool {
        self.cancellation_token.lock().expect("cancellation_token mutex poisoned").is_cancelled()
    }
    ...
    pub fn start_reconnecting(&self, device: Arc<AudioDevice>, device_type: DeviceType) {
        ...
        *self.disconnected_device.lock().expect("disconnected_device mutex poisoned") = Some((device, device_type));
    ...
    pub fn get_disconnected_device(&self) -> Option<(Arc<AudioDevice>, DeviceType)> {
        self.disconnected_device.lock().expect("disconnected_device mutex poisoned").clone()
    }

    // Device management
    pub fn set_microphone_device(&self, device: Arc<AudioDevice>) {
        *self.microphone_device.lock().expect("microphone_device mutex poisoned") = Some(device);
    }

    pub fn set_system_device(&self, device: Arc<AudioDevice>) {
        *self.system_device.lock().expect("system_device mutex poisoned") = Some(device);
    }

    pub fn get_microphone_device(&self) -> Option<Arc<AudioDevice>> {
        self.microphone_device.lock().expect("microphone_device mutex poisoned").clone()
    }

    pub fn get_system_device(&self) -> Option<Arc<AudioDevice>> {
        self.system_device.lock().expect("system_device mutex poisoned").clone()
    }

    // Audio pipeline management
    pub fn set_audio_sender(&self, sender: mpsc::UnboundedSender<AudioChunk>) {
        *self.audio_sender.lock().expect("audio_sender mutex poisoned") = Some(sender);
    }
    ...
    pub fn send_audio_chunk(&self, chunk: AudioChunk) -> Result<()> {
        ...
        if let Some(sender) = self.audio_sender.lock().expect("audio_sender mutex poisoned").as_ref() {
    ...
    {
        *self.error_callback.lock().expect("error_callback mutex poisoned") = Some(Box::new(callback));
    }
    ...
    pub fn report_error(&self, error: AudioError) {
        ...
        *self.last_error.lock().expect("last_error mutex poisoned") = Some(error.clone());

        // Call error callback if set
        if let Some(callback) = self.error_callback.lock().expect("error_callback mutex poisoned").as_ref() {
            callback(&error);
        }
    ...
    pub fn has_fatal_error(&self) -> bool {
        if let Some(error) = &*self.last_error.lock().expect("last_error mutex poisoned") {
    ...
    pub fn get_recording_duration(&self) -> Option<f64> {
        self.recording_start
            .lock()
            .expect("recording_start mutex poisoned")
            .map(|start| start.elapsed().as_secs_f64())
    }

    pub fn get_active_recording_duration(&self) -> Option<f64> {
        // Snapshot values first to avoid nested locking (which would deadlock on std::sync::Mutex)
        let recording_start = self.recording_start.lock().expect("recording_start mutex poisoned").clone();
        let is_paused = self.is_paused();
        let pause_start = self.pause_start.lock().expect("pause_start mutex poisoned").clone();
        let total_pause_duration = self.total_pause_duration.lock().expect("total_pause_duration mutex poisoned").as_secs_f64();

        recording_start.map(|start| {
            let total_duration = start.elapsed().as_secs_f64();
            let current_pause = if is_paused {
                pause_start
            ...
            total_duration - total_pause_duration - current_pause
        })
    }

    pub fn get_total_pause_duration(&self) -> f64 {
        self.total_pause_duration.lock().expect("total_pause_duration mutex poisoned").as_secs_f64()
    }
    ...
    pub fn get_current_pause_duration(&self) -> Option<f64> {
        if self.is_paused() {
            self.pause_start
                .lock()
                .expect("pause_start mutex poisoned")
                .map(|start| start.elapsed().as_secs_f64())
        } else {
            None
        }
    ...
    pub fn cleanup(&self) {
        ...
        *self.audio_sender.lock().expect("audio_sender mutex poisoned") = None;
        *self.last_error.lock().expect("last_error mutex poisoned") = None;
        *self.error_callback.lock().expect("error_callback mutex poisoned") = None;
        *self.stats.lock().expect("stats mutex poisoned") = RecordingStats::default();
        *self.recording_start.lock().expect("recording_start mutex poisoned") = None;
        *self.pause_start.lock().expect("pause_start mutex poisoned") = None;
        *self.total_pause_duration.lock().expect("total_pause_duration mutex poisoned") = std::time::Duration::ZERO;
        self.error_count.store(0, Ordering::SeqCst);
        self.recoverable_error_count.store(0, Ordering::SeqCst);
