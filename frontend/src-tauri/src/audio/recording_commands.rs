...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    if let Some(lifecycle_manager) = app.try_state::<std::sync::Arc<crate::lifecycle::AppLifecycleManager>>() {
        manager.set_lifecycle_manager(lifecycle_manager.inner().clone());
    }
    ...
    {
        let mut global_manager = RECORDING_MANAGER.lock().expect("recording_manager poisoned");
    ...
    {
        let mut global_task = TRANSCRIPTION_TASK.lock().expect("transcription_task poisoned");
    ...
    // Store listener ID for cleanup during stop_recording to ensure microphone is released
    {
        ...
        let mut global_listener = TRANSCRIPT_LISTENER_ID.lock().expect("transcript_listener_id poisoned");
...
pub async fn stop_recording<R: Runtime>(
    ...
    {
        use tauri::Listener;
        if let Some(listener_id) = TRANSCRIPT_LISTENER_ID.lock().expect("transcript_listener_id poisoned").take() {
        ...
        Some("parakeet") => {
            ...
            let engine_clone = {
                let engine_guard = crate::parakeet_engine::commands::PARAKEET_ENGINE
                    .lock()
                    .expect("parakeet_engine poisoned");
        ...
        _ => {
            ...
            let engine_clone = {
                let engine_guard = crate::whisper_engine::commands::WHISPER_ENGINE
                    .lock()
                    .expect("whisper_engine poisoned");
                engine_guard.as_ref().cloned()
            };
    ...
    // Step 3.5: Analytics tracking removed (privacy-first approach)
...
pub async fn pause_recording<R: Runtime>(app: AppHandle<R>) -> Result<(), String> {
    ...
    let manager_guard = RECORDING_MANAGER.lock().expect("recording_manager poisoned");
    if let Some(manager) = manager_guard.as_ref() {
...
pub async fn poll_audio_device_events() -> Result<Option<DeviceEventResponse>, String> {
    let mut manager_guard = RECORDING_MANAGER.lock().expect("recording_manager poisoned");
