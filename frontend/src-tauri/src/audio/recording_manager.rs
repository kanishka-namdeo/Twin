...
impl RecordingManager {
    ...
    /// Set the lifecycle manager for this recording manager
    ...
    pub async fn start_recording(
        ...
        // Register with lifecycle manager for ordered shutdown
        if let Some(ref lifecycle_manager) = self.lifecycle_manager {
            let state_clone = self.state.clone();
            lifecycle_manager
                .register(
                    "recording_manager",
                    crate::lifecycle::resource::CleanupPriority::Critical,
                    Box::new(move || {
                        Box::pin(async move {
                            log::info!("Lifecycle: cleaning up recording manager...");
                            state_clone.stop_recording();
                            state_clone.cleanup();
                            log::info!("Lifecycle: recording manager cleanup complete");
                            Ok(())
                        })
                    }),
                )
    ...
    pub async fn stop_streams_only(&mut self) -> Result<()> {
        ...
        if let Some(ref lifecycle_manager) = self.lifecycle_manager {
            lifecycle_manager.unregister("recording_manager").await;
        }
...
impl Drop for RecordingManager {
    fn drop(&mut self) {
        log::debug!("RecordingManager::drop() called");
        
        // Attempt to unregister from lifecycle manager (best effort)
        if let Some(ref lifecycle_manager) = self.lifecycle_manager {
            let manager = lifecycle_manager.clone();
            if let Ok(rt) = tokio::runtime::Handle::try_current() {
                rt.spawn(async move {
                    manager.unregister("recording_manager").await;
                });
            }
        }
        
        // Stop recording state to release audio devices
        self.state.stop_recording();
        
        // Clean up any remaining resources
        self.state.cleanup();
        
        log::debug!("RecordingManager::drop() completed");
    }
}