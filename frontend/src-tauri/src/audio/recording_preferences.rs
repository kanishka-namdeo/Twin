...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\twin-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("twin-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("twin-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/twin-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("twin-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/twin-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("twin-recordings")
    }
