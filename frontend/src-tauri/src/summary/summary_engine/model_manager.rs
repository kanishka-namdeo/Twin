...
pub struct ModelManager {
    ...
    /// Lifecycle manager for resource tracking
...
impl ModelManager {
    ...
    pub async fn download_model_detailed(
        ...
        // Register download with lifecycle manager
        if let Some(ref lifecycle_manager) = self.lifecycle_manager {
            let cancel_flag = self.cancel_download_flag.clone();
            let model_name_owned = model_name.to_string();
            lifecycle_manager
                .register(
                    format!("model_download_{}", model_name),
                    crate::lifecycle::resource::CleanupPriority::Secondary,
                    Box::new(move || {
                        Box::pin(async move {
                            log::info!("Lifecycle: cancelling download for model '{}'", model_name_owned);
                            let mut flag = cancel_flag.write().await;
                            *flag = Some(model_name_owned.clone());
                            log::info!("Lifecycle: download cancellation flag set for '{}'", model_name_owned);
                            Ok(())
                        })
                    }),
                )
        ...
        if file_path.exists() {
            if let Ok(metadata) = fs::metadata(&file_path).await {
                ...
                if file_size_mb >= expected_min && file_size_mb <= expected_max {
                    ...
                    if let Some(ref lifecycle_manager) = self.lifecycle_manager {
                        lifecycle_manager.unregister(&format!("model_download_{}", model_name)).await;
                    }
        ...
        loop {
            // Check for cancellation
            {
                ...
                if cancel_flag.as_ref() == Some(&model_name.to_string())
                    || cancel_flag.as_ref() == Some(&"__DROPPED__".to_string())
                {
    ...
    /// Set the lifecycle manager for this model manager
...
impl Drop for ModelManager {
    fn drop(&mut self) {
        // Set cancellation flag to stop any active downloads
        // Note: We can't await in Drop, but we can set the flag synchronously
        // The download loop will detect it on its next iteration
        if let Ok(mut flag) = self.cancel_download_flag.try_write() {
            if flag.is_some() {
                log::info!("ModelManager dropped, setting cancellation flag for active download");
                // We can't get the model name here without awaiting, so we'll set a special marker
                *flag = Some("__DROPPED__".to_string());
            }
        }
        log::info!("ModelManager dropped");
    }
}
