...
export function TranscriptProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // CRITICAL FIX: Cap transcript array size to prevent unbounded memory growth
    // during long meetings. Keep the last 1000 transcripts (roughly 2-3 hours of speech).
    ...
    const processBufferedTranscripts = (forceFlush = false) => {
      ...
      if (allNewTranscripts.length > 0) {
        setTranscripts(prev => {
          ...
          // CRITICAL FIX: Trim old transcripts if we exceed the limit to prevent memory leaks
          // Keep only the most recent MAX_TRANSCRIPTS entries
          const trimmed = combined.length > MAX_TRANSCRIPTS
            ? combined.slice(combined.length - MAX_TRANSCRIPTS)
            : combined;
