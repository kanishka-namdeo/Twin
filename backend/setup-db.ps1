...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.twin\meeting_minutes.db",
        "$env:USERPROFILE\Documents\twin\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/twin-backend/*/backend/meeting_minutes.db"
    }
