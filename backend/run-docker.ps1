...
$AppProjectName = "twin-backend"
$AppContainerName = "twin-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "twin-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server twin-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server twin-backend
...
function Invoke-LogsCommand {
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "twin-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, twin-backend"
        ...
        $logArgs += @("whisper-server", "twin-backend")
    }
