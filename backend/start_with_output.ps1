...
Write-Host "====================================="
Write-Host "Twin Backend Startup"
Write-Host "====================================="
...
if (-not (Test-Path "whisper-server-package\whisper-server.exe")) {
    ...
    try {
        ...
        $apiUrl = "https://api.github.com/repos/kanishka-namdeo/Twin/releases/latest"
        ...
        $downloadUrl = "https://github.com/kanishka-namdeo/Twin/releases/download/$tagName/whisper-server.exe"
        ...
        try {
            $allReleasesUrl = "https://api.github.com/repos/kanishka-namdeo/Twin/releases"
            $headers = @{"User-Agent" = "PowerShell-Script"}
            ...
            if ($releases.Count -gt 0) {
                ...
                $altDownloadUrl = "https://github.com/kanishka-namdeo/Twin/releases/download/$latestTag/whisper-server.exe"
            ...
            Write-Host "https://github.com/kanishka-namdeo/Twin/releases"
...
Write-Host "====================================="
Write-Host "Starting Twin Backend"
Write-Host "====================================="
...
# Check if twin-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for twin-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\twin-frontend\twin-frontend.exe",
    "$env:LOCALAPPDATA\Programs\twin\twin-frontend.exe",
    "$env:ProgramFiles\twin-frontend\twin-frontend.exe",
    "${env:ProgramFiles(x86)}\twin-frontend\twin-frontend.exe",
    "$env:APPDATA\twin-frontend\twin-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("twin-frontend.exe", "twin.exe")
...
if ($frontendInstalled) {
    Write-Host "Twin frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the Twin frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching Twin frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "Twin frontend launched successfully."
        }
    }
} else {
    Write-Host "Twin frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the Twin frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if twin is now installed and launch it
